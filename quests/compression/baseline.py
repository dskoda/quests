...
def dbscan_sample(descriptors, entropies: np.ndarray, size: int):
    ...
    optimizer = BayesianOptimization(
        f=cost_fn, pbounds=bounds, allow_duplicate_points=True
    )
    optimizer.set_gp_params
    optimizer.maximize(init_points=15, n_iter=15)

    dbscan = DBSCAN(eps=optimizer.max["params"]["x"], min_samples=1).fit(
        avg_descriptors
    )
