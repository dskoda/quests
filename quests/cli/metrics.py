...
from quests.entropy import DEFAULT_BANDWIDTH, DEFAULT_BATCH, get_all_metrics
...
def metrics(
    ...
    with Timer() as t:
        entropy, diversity, dH = get_all_metrics(x, h=bandwidth, batch_size=batch_size)
    entropy_time = t.time
    logger(f"Metrics computed in: {format_time(entropy_time)}")

    logger(f"Dataset entropy: {entropy: .3f} (nats)")
    logger(f"Dataset diversity: {diversity: .3f} (nats)")
